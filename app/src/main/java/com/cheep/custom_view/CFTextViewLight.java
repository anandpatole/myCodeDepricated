...
/**
 * Custom Textview for light font, including feature setting up letterSpacing dynamically
 */
public class CFTextViewLight extends AppCompatTextView {

    private Context mContext;

    public CFTextViewLight(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        mContext = context;
        init(attrs, defStyle);
    }

    public CFTextViewLight(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init(attrs, 0);
    }

    public CFTextViewLight(Context context) {
        super(context);
        mContext = context;
        init();
    }
    ...
    private void init() {
        setTypeface(TypeFaceProvider.get(mContext, getResources().getString(R.string.font_light)));
    }
